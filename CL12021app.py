...
def generate_pdf_details(meetings_df, groups_df, period_str, start_date, end_date):
    ...
    for range_name, min_part, max_part in participant_ranges:
        ...
        for gtype in ['DGE', 'Supervision', 'Junior']:
            ...
            # Byg tabel data
            table_data = [['Dato', 'Gruppenavn', 'Supervisor', 'Mødetype', 'Antal']]
            
            for _, row in gtype_meetings.iterrows():
                date_str = row['Starttidspunkt'].strftime('%d-%m-%Y')
                group_name = str(row['Gruppenavn'])[:35]
                supervisor = str(row.get('Supervisor', '-'))[:25]
                meeting_type = str(row['Mødetype'])[:20]
                participants = str(int(row['Antal deltagere']))
                
                table_data.append([date_str, group_name, supervisor, meeting_type, participants])
            
            # Opret tabel
            table = Table(table_data, colWidths=[1*inch, 2.5*inch, 1.8*inch, 1.3*inch, 0.6*inch])
    ...
    for num_meetings in [0, 1, 2, 3, 4]:
        ...
        for gtype in ['DGE', 'Supervision', 'Junior']:
            gtype_groups = groups_this_count[groups_this_count['Gruppetype_std'] == gtype].copy()
            
            if gtype_groups.empty:
                continue
            
            story.append(Paragraph(f"<b>{gtype}</b>", styles['Heading3']))
            
            # Byg tabel data
            table_data = [['Gruppenavn', 'Supervisor', 'Status', 'Gruppetype', 'Antal møder']]
            
            for _, row in gtype_groups.iterrows():
                group_name = str(row['Gruppenavn'])[:40]
                supervisor = str(row.get('Supervisor', '-'))[:25]
                
                # Bestem lukket status
                arkiv_dato = row.get('Dato for arkivering')
                if pd.notna(arkiv_dato):
                    if start_date <= arkiv_dato <= end_date:
                        # Lukket i perioden - RØD
                        status_text = f"<font color='red'>LUKKET {arkiv_dato.strftime('%d-%m-%Y')}</font>"
                    else:
                        # Lukket efter perioden - GUL
                ...
                table_data.append([group_name, supervisor, Paragraph(status_text, styles['Normal']), gruppetype, antal])
            
            # Opret tabel
            table = Table(table_data, colWidths=[2.5*inch, 1.5*inch, 1.8*inch, 1*inch, 0.7*inch])
