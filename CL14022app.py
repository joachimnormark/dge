    ...
    if not meetings_df.empty:
        ...
        if not supervision_meetings.empty:
            ...
            dge_with_supervision = supervision_meetings[
                supervision_meetings['Gruppetyper'].astype(str).str.contains('DGE', case=False, na=False)
            ].copy()
            
            if not dge_with_supervision.empty:
                # Tæl antal supervision møder per gruppe
                supervision_counts = dge_with_supervision.groupby('Gruppenavn').size().reset_index(name='Antal_supervision_møder')
                
                # Merge med supervisor info
                supervision_counts = supervision_counts.merge(
                    group_types[['Gruppenavn', 'Supervisor']], 
                    on='Gruppenavn', 
                    how='left'
                )
...
def generate_pdf_supervisor_report(supervisor_name, groups_df, meetings_df, period_str, start_date, end_date):
    ...
    supervisor_groups = supervisor_groups.sort_values(['Gruppetype_std', 'Gruppenavn'])
