...
def filter_members_gruppeledere(seats_df):
    ...
    if 'Medlemskaber' in df.columns:
        # Konverter til string mere robust - håndter ArrowDtype og NaN
        df['Medlemskaber'] = df['Medlemskaber'].fillna('').astype(str)
        
        def filter_gruppeledere_membership(medlemskaber_str):
            if not medlemskaber_str or medlemskaber_str == 'nan' or medlemskaber_str == '':
                return ''
            
            try:
                filtered = [
                    g.strip() 
                    for g in str(medlemskaber_str).split(',') 
                    if g.strip() and 'gruppeledere' not in g.strip().lower()
                ]
                return ','.join(filtered)
            except:
                return str(medlemskaber_str)
        
        df['Medlemskaber'] = df['Medlemskaber'].apply(filter_gruppeledere_membership)
...
# VERSION NUMMER
APP_VERSION = "v11.1 - Fix: ArrowDtype håndtering"
