...
def generate_pdf_with_charts(all_charts, period_info):
    ...
    for i, (fig, title, desc) in enumerate(all_charts):
        ...
        try:
            ...
            img_obj = RLImage(img_buffer, width=7*inch, height=3.5*inch)
        ...
        for _, row in groups_this_count.iterrows():
            ...
            if pd.notna(arkiv_dato):
                if start_date <= arkiv_dato <= end_date:
                    ...
                    status_text = f"<font color='red'><b>LUKKET {arkiv_dato.strftime('%d-%m-%Y')}</b></font>"
                else:
                    # Lukket efter perioden - GUL/ORANGE
                    status_text = f"<font color='#DAA520'><b>LUKKET EFTER PERIODEN {arkiv_dato.strftime('%d-%m-%Y')}</b></font>"
            else:
                status_text = "Aktiv"
